...
import { DistributorPriceInfoService } from "src/app/Services/Distributor/DistributorPriceInfo/distributor-price-info.service";

@Component({
  selector: 'app-distributor-price-info',
  templateUrl: './distributor-price-info.component.html',
  styleUrls: ['./distributor-price-info.component.css']
})
export class DistributorPriceInfoComponent implements OnInit {
  formField: any;
  ...
  constructor(
    private _DistributorPriceInfoService: DistributorPriceInfoService,
  ...
  submit(Data) {
    ...
    Data.MethodName = "InUp_DistributorPriceInfo";
    delete Data["CategoryID"];
    delete Data["ProductName"];
    this._DistributorPriceInfoService.AddDistributorPrice(Data).subscribe({
      next: data => {
  ...
  getList() {
    this._DistributorPriceInfoService.GetDistributorPrice(this.getRequestBody()).subscribe({
      next: data => {
        this.priceList = data;
      },
      error: error => {
  ...
  public getRequestBody() {
    return {
      MethodName: "Search_DistributorPriceInfo"
    }
  }
